  ...
  e === "top"
    ...
    : (() => {
        ...
        window.scrollTo({
          top:
            document
              .querySelector("#" + _.kebabCase(e.target.innerText))
...
export default function Home() {
  ...
  useEffect(() => {
    if (!content) return setAllAuthors(getAllAuthorsInfo(content));
  }, [content]);
  ...
  return (
    <div className="relative min-h-screen max-w-full">
      <Head>
        ...
        <meta
          name="description"
          content="We make jerma sites for people to enjoy (or hate) of varying quality."
        />
      ...
      <main className="pb-40">
        ...
        <div className="bg-jerma-pink text-jerma-deep-blue text-center py-4">
          📣 You can contribute on{" "}
          <Link href="https://github.com/JermaSites" className="font-bold">
            Github!
          </Link>
        </div>
        ...
        <div className="flex m-auto max-w-7xl flex-col md:flex-row-reverse">
          <div className="max-w-xs mx-auto">
            <div
              ref={jermaLogo}
              className="select-none mt-8 flex justify-center"
            >
            ...
            <div className="text-jerma-light-blue text-center mt-8 sticky top-20">
              ...
              <div className="pt-8 sticky scale-90 top-12 bg-jerma-deep-blue z-10">
                <h2 className="text-2xl mx-2 text-jerma-pink font-semibold pb-2 text-center md:text-left border-b-[1px] border-jerma-pink">
                  Collaborators
                </h2>
              </div>
              <div className="mx-2 -mt-6 scale-90">
                  ...
                  (author) =>
                    author.avatar && (
                      <Link
                        ...
                        className="no-underline  hover:opacity-100 flex relative items-center py-2 border-b border-jerma-light-blue/10 m:hover:bg-slate-500/5"
                        href={author.url}
                      >
                        {author.avatar && (
                          <div className="relative h-12 w-14 overflow-hidden rounded-full">
                            <img src={author.avatar} alt="" layout="fill" />
                          </div>
                        )}
                        <p className="ml-3 text-left w-full">
                          <span
                            className={`${author.project_count > 5 && "gold"}`}
                          >
                            {author.name}
                          </span>{" "}
                          <br />{" "}
                          <span className={`opacity-70 text-sm`}>
                            {author.project_count} collab
                            {author.project_count > 1 && "s"}
                          </span>
                        </p>
                      </Link>
                    )
                ...
                <Link
                  ...
                  className="flex items-center w-full text-center text-jerma-pink no-underline mt-8 md:mt-4 font-semibold justify-end scale-110 -translate-x-8 md:translate-x-0  md:scale-100"
                >
                  <p className="mt-[2px] mr-2">Join us</p>
                  <Plus className="fill-jerma-pink" />
                </Link>
              </div>
            </div>
          </div>
          ...
          <div>
            {content &&
              content.sections.map((section, i) => (
                <div
                  key={_.kebabCase(section.title) + i}
                  id={_.kebabCase(section.title)}
                  className="m-auto mx-0 sm:mx-4"
                >
                  <div className="pt-16 sticky top-4 bg-jerma-deep-blue z-10">
                    <h2 className="text-2xl text-jerma-pink font-semibold pb-2 text-center md:text-left border-b-[1px] border-jerma-pink">
                      {section.title}
                    </h2>
                  </div>
                  ...
                  {section.posts.map((post, i) => (
                    <div
                      key={`${post.title}${i}`}
                      className="flex-wrap p-4 text-jerma-light-blue flex justify-between border-b border-jerma-light-blue/50 odd:bg-black/5 last:border-b-0 py-4 sm:py-4 no-underline hover:opacity-100"
                    >
        ...
        {post.authors.map((author, i) => (
          <div key={author.name + i}>
            <h2 className="font-light text-3xl text-jerma-pink">
              {author.url ? (
                <Link href={author.url} className="no-under-link">
                  {author.name}
                </Link>
              ) : (
                author.name
  ...
  return post.authors
    ...
    .map((author, i, posts) => {
      if (i == posts.length - 1) {
        return author.url ? (
          <Link
            className="font-bold text-jerma-bright-blue whitespace-nowrap md:text-sm"
            href={author.url}
          >
            {author.name}
          </Link>
